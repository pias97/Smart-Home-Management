...
void loop() {
 ...
 if(button1 == HIGH)
 {
  ...
  sendValue(RECEIVER1_ID, x); // sending data to receiver A
 }
 if(button2 == HIGH)
 {
   sendValue(RECEIVER2_ID, x); // sending data to receiver B
 }
 if(button3 == HIGH)
 {
  sendValue(RECEIVER3_ID, x); // send data to receiver C
 }
 delay(1000);
}
